...
- ⚠️ ویندوز (10.10.1.5): اینترفیسِ خصوصی درونِ‌OS احتمالاً هنوز دستی کانفیگ نشده (ICMP جواب نداد ولی 3389 روی خصوصی باز است).
...
- **دسترسی:** RDP:3389 و **WinRM:5985 (HTTP)** باز؛ SSH ندارد. کاربر: `Administrator`، رمز از `WIN_ADMIN_PASS` زمانِ ساخت (build_base.py) — **روی فارکس ذخیره نشده**.
- **باقی‌ماندهٔ ممیزیِ درونِ‌OS (نیازِ رمزِ Administrator):** سرویس‌ها/Taskها، تعدادِ ترمینال‌های MT5 در حالِ اجرا، مسیرِ `C:\CoinePro\users`، نسخهٔ MT5. ابزارش آماده است: کلاینتِ WinRM (`pypsrp`) روی سرورِ مرکزی نصب شد؛ به‌محضِ گرفتنِ رمز، `server-audit.ps1` را headless اجرا می‌کنم.
