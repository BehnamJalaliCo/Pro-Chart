...
## رفع ۱۳ — دسته‌های کناریِ دیالوگِ اندیکاتور روی تب‌های بدون‌کاتالوگ کنترلِ مرده بودند (Loop #114)
فایل: `frontend/prochart/src/bazaarnama/IndicatorsDialog.jsx` — سایدبارِ دسته + دکمهٔ تب.
باگ: روی تبِ استراتژی/پروفایل/الگو (که کاتالوگِ per-category ندارند، خط ۲۱۱ `base=[]`)، دسته‌های کناری (تکنیکال با شمارِ «108» و…) همچنان کلیک‌پذیر + هایلایت می‌شدند ولی کاری نمی‌کردند (کنترلِ مرده/گمراه‌کننده). ضمناً `cat` با تعویضِ تب ریست نمی‌شد ⇒ بازگشت به تبِ اندیکاتور روی دستهٔ خالیِ قبلی (مثلِ منتخب‌ها) می‌افتاد.
رفع (فرانت، افزایشی): (۱) سایدبار روی `tab !== 'indicators'` → `opacity-40 pointer-events-none` + `aria-hidden` (سبکِ TV: دسته‌های نامرتبط خاکستری). (۲) `setTab` حالا `setCat('technicals')` هم می‌زند تا بازگشت به اندیکاتور همیشه Technicals باشد.
تأییدِ زنده (`index-DTPYKp8q.js`): تبِ اندیکاتور سایدبار `opacity=1, pointer-events=auto`؛ تبِ استراتژی `opacity=0.4, pointer-events=none`؛ ۰ خطای JS.
...
## رفع ۱۵ — رشته‌های انگلیسیِ ترجمه‌نشده در دو سطحِ زنده (فارسیِ سلیس، Loop #123)
دو رشتهٔ کاربر-رو که برای کاربرِ فارسی انگلیسی مانده بودند (سوئیپِ کیفیتِ فارسی با subagent؛ موارد عمدیِ دوزبانه مثلِ «SCRIPT NAME»ِ TV و لیبل‌های ثانویهٔ Details کنار گذاشته شدند چون آینهٔ هدرهای واقعیِ TradingView‌اند).
- فایل: `frontend/prochart/src/i18n.js` — کلیدِ نو `lock.ok` افزوده شد (fa: «تأیید»، en: «OK»). این فایل تغییرِ WIPِ نشستِ قبل هم دارد (de-brandingِ `ob.d1`) پس این‌جا کامیت نمی‌شود.
- فایل: `frontend/prochart/src/app/AppLock.jsx` — دکمهٔ «OK»ِ صفحه‌کلیدِ PIN (که کاربر با آن اپ را باز می‌کند) به `{t('lock.ok')}` تبدیل شد؛ حالا فارسی/دوزبانه است و با بقیهٔ دکمه‌ها (که از t() استفاده می‌کنند) سازگار. این فایل تغییرِ WIPِ نشستِ قبل هم دارد (importِ آیکون‌ها از lucide-react → tvIcons) پس این‌جا کامیت نمی‌شود.
- (لیبلِ چارت‌اپِ `Bar-by-bar`/`Vectorized` → «بار-به-بار»/«برداری» در `pages/BazaarNama.jsx` تمیز بود و جداگانه کامیت شد.)
تأییدِ زنده (`index-tuQRTF5B.js`): باندلِ سروشده شاملِ کلیدِ نوِ `lock.ok` + «بار-به-بار»/«برداری» است (اثباتِ بیلدِ تازه، نه کش).

---

## رفع ۱۶ — دکمهٔ پاک‌کردنِ جستجوی واچ‌لیست بدونِ tooltip/aria (Loop #125)
سوئیپِ subagent روی دکمه‌های صرفاً-آیکونیِ کلِ چارت‌اپ فقط **۲ دکمهٔ بی‌برچسب** یافت (بقیه faithful) — هر دو دکمهٔ X:
- فایل: `frontend/prochart/src/bazaarnama/RightPanel.jsx` خط ۷۳۲ — دکمهٔ پاک‌کردنِ کوئریِ جستجوی «افزودنِ نماد» در واچ‌لیست، صرفاً `<X size={12}/>` بدونِ `title`/`aria-label` بود → `title`+`aria-label`="پاک‌کردنِ جستجو" افزوده شد (hover-tooltipِ سبکِ TV + نامِ در-دسترس). این فایل ~۲۳۹ خط تغییرِ WIPِ نشست‌های قبل دارد (importها، ترتیبِ ستون‌ها، `sepThousands`) پس این‌جا کامیت نمی‌شود؛ live-uncommitted.
- (دکمهٔ دومِ بی‌برچسب — بستنِ دیالوگِ راهنما در `bazaarnama/Help.jsx` خط ۴۵۲ — فایلش تمیز بود و جداگانه کامیت شد: `title`+`aria-label`="بستنِ راهنما".)
تأییدِ زنده (`index-N-Wv4b09.js`): باندلِ سروشده «پاک‌کردنِ جستجو» + «بستنِ راهنما» دارد.

---

**وضعیت:** هر شانزده رفع در working-tree اعمال و زنده دیپلوی شده‌اند (بیلدهای این نشست). فقط منتظرِ snapshot-commitِ مالک برای ثبت در گیت‌اند.
