...
> وضعیت: 📝 **طرح — منتظرِ تأییدِ نهاییِ مالک برای دیپلوی.** اسکلتِ کد ساخته شده (`central-auth/`)؛ هیچ چیزی روی سرورِ زنده نصب/تغییر نشده.
> پیش‌نیازِ اجرا: رمزِ sudo روی سرورِ مرکزی (برای نصبِ Docker) — چون کاربرِ `Hub` بدونِ sudo است.
