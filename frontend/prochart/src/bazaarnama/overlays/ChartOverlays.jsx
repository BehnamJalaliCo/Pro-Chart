...
export function Legend({ legend, TH, symbol, tf }) {
  ...
  return (
    <div className="absolute top-2 right-2 z-20 text-[11px] rounded-md px-2.5 py-1 font-mono tabular-nums flex items-center gap-2.5 border shadow-sm" dir="ltr" style={{ background: TH.overlayMask, borderColor: TH.border, backdropFilter: 'blur(3px)' }}>
      ...
      {ch != null && <span className="font-semibold" style={{ color: col }}>{ch >= 0 ? '+' : ''}{ch.toFixed(2)}%</span>}
    </div>
  );
}
...
function LegendIconBtn({ TH, title, ariaLabel, onClick, danger, children, alwaysShow }) {
  return (
    <button
      ...
      style={{ width: 22, height: 22, color: TH.text }}
...
function LegendRow({ item, TH, value, coarse, viewMode, onToggle, onSettings, onRemove, onMore, moreOpen, onCloseMore, onDup, onHelp, hasHelp, onPinScale }) {
  ...
  if (viewMode === 'compact') {
    return (
      <button type="button" onClick={() => onSettings(item)} title={item.label}
        className="flex items-center gap-1.5 rounded-md px-2 h-6 text-[12px] font-medium shrink-0 border transition-colors duration-[120ms]"
        style={{ background: TH.chipBg, borderColor: TH.border, color: TH.textStrong, opacity: visible ? 1 : 0.45 }}
  ...
  return (
    <div className="group/leg flex items-center gap-2 rounded-md px-1.5 h-7 relative transition-colors duration-[120ms]"
        ...
        <div className="relative">
          ...
          {moreOpen && (
            <>
              ...
              <div className="absolute left-0 top-8 z-[61] w-44 rounded-lg py-1 text-[12px] shadow-xl overflow-hidden" dir="rtl"
...
export function ChartLegend({
  ...
  return (
    <div className="absolute top-2 right-2 z-20 max-w-[min(70%,520px)] shadow-sm"
      style={{ background: TH.overlayMask, backdropFilter: 'blur(3px)', border: `1px solid ${TH.border}`, borderRadius: 8, padding: '4px 6px' }}>
      ...
      <div className="flex items-center gap-2 px-1.5 h-7">
        {hasInds && (
          <button type="button" onClick={() => onCollapse && onCollapse(!collapsed)} title={collapsed ? 'بازکردنِ اندیکاتورها' : 'جمع‌کردنِ اندیکاتورها'} aria-label="جمع/باز"
            className="flex items-center justify-center rounded-md shrink-0 transition-colors duration-[120ms]" style={{ width: 18, height: 18, color: TH.text }}
        ...
        {onSymbolSettings && (
          <button type="button" onClick={onSymbolSettings} title="تنظیماتِ چارت" aria-label="تنظیماتِ چارت"
            className="flex items-center justify-center rounded-md shrink-0 opacity-50 hover:opacity-100 transition-all duration-[120ms]" style={{ width: 18, height: 18, color: TH.text }}
            ...
            <div className="flex items-center gap-0.5 shrink-0">
              {onToggleIndsHidden && (
                <button type="button" onClick={onToggleIndsHidden} title={indsHidden ? 'نمایشِ همهٔ اندیکاتورها' : 'پنهان‌کردنِ موقتِ همهٔ اندیکاتورها'} aria-label="نمایش/پنهانِ همهٔ اندیکاتورها"
                  className="flex items-center justify-center rounded-md shrink-0 transition-colors duration-[120ms]" style={{ width: 22, height: 22, color: indsHidden ? TH.accent : TH.text }}
              ...
              <button type="button" onClick={onToggleViewMode} title={compact ? 'نمای کامل' : 'نمای فشرده'} aria-label="حالتِ نمایش"
                className="flex items-center justify-center rounded-md shrink-0 transition-colors duration-[120ms]" style={{ width: 22, height: 22, color: compact ? TH.accent : TH.text }}
              ...
              <button type="button" onClick={() => { if (window.confirm('همهٔ اندیکاتورها حذف شوند؟')) onClearAll && onClearAll(); }} title="پاکِ همهٔ اندیکاتورها" aria-label="حذفِ همهٔ اندیکاتورها"
                className="flex items-center justify-center rounded-md shrink-0 transition-colors duration-[120ms]" style={{ width: 22, height: 22, color: TH.text }}
      ...
      {hasInds && !collapsed && (
        compact ? (
          ...
          <div className="flex flex-col mt-0.5 pt-0.5" style={{ borderTop: `1px solid ${TH.border}` }}>
            {items.map((it) => (
...
export function CountdownChip({ countdown, countdownColor, TH, marketOpen, frac = 0 }) {
  ...
  return (
    <div className="absolute bottom-3 right-3 z-20 pointer-events-none rounded-md px-2.5 py-1 text-[11px] font-mono tabular-nums flex items-center gap-2 border shadow-md" style={{ borderColor: countdownColor || TH.border, background: TH.popoverBg, color: countdownColor || undefined, backdropFilter: 'blur(3px)' }} dir="ltr">
...
export function ReplayBar({ replay, TH, replayStepBack, replayToggle, replayStep, replaySeek, replaySetSpeed, exitReplay }) {
  ...
  return (
    <div className="absolute bottom-3 left-1/2 -translate-x-1/2 z-20 flex items-center gap-2.5 px-3 py-1.5 rounded-xl text-xs shadow-md" style={{ background: TH.popoverBg, border: `1px solid ${TH.border}`, backdropFilter: 'blur(3px)' }}>
      <span className="font-bold" style={{ color: TH.accent }}>بازپخش</span>
      <div className="flex items-center gap-1">
        <button onClick={replayStepBack} title="گامِ عقب" className="flex items-center justify-center w-7 h-7 rounded-md transition-colors duration-[120ms]" style={{ background: TH.chipBg }} onMouseEnter={(e) => (e.currentTarget.style.background = TH.chipBgHover)} onMouseLeave={(e) => (e.currentTarget.style.background = TH.chipBg)}>⏮</button>
        <button onClick={replayToggle} title="پخش/مکث" className="flex items-center justify-center w-7 h-7 rounded-md text-white transition-colors duration-[120ms]" style={{ background: TH.accent }}>{replay.playing ? '⏸' : '▶'}</button>
        <button onClick={replayStep} title="گامِ جلو" className="flex items-center justify-center w-7 h-7 rounded-md transition-colors duration-[120ms]" style={{ background: TH.chipBg }} onMouseEnter={(e) => (e.currentTarget.style.background = TH.chipBgHover)} onMouseLeave={(e) => (e.currentTarget.style.background = TH.chipBg)}>⏭</button>
      </div>
      <span className="w-px h-5 shrink-0" style={{ background: TH.border }} />
      ...
      <div className="flex items-center gap-0.5">
        {SPEED_LADDER.map((sp) => (<button key={sp} onClick={() => replaySetSpeed(sp)} className={`px-1.5 py-0.5 rounded-md tabular-nums transition-colors duration-[120ms] ${replay.speed === sp ? 'text-white' : 'opacity-60 hover:opacity-100'}`} style={replay.speed === sp ? { background: TH.accent } : {}}>{sp}×</button>))}
      </div>
      <span className="w-px h-5 shrink-0" style={{ background: TH.border }} />
      <button onClick={exitReplay} className="px-2 py-1 rounded-md bg-red-500/20 text-red-400 hover:bg-red-500/30 transition-colors duration-[120ms]">✕ خروج</button>
    </div>
  );
}
