 ...
 */
export const crosshairOptions = (id, magnet, th, color, width, style, labels = true) => {
  ...
  const T = th || {};
  const lineColor = color || T.text || '#9598a1'; // رنگِ سفارشیِ کراس‌هیر (اختیاری، مثلِ Chart Settings تریدینگ‌ویو)
  const labelBg = color || T.accent || '#2962FF'; // رنگِ سفارشیِ کراس‌هیر روی پس‌زمینهٔ برچسبِ محور هم اعمال می‌شود
  const lineCfg = {
    visible: m.linesVisible,
    color: lineColor,
    width: width || 1,                        // ضخامتِ سفارشیِ کراس‌هیر (۱..۳)
    style: style != null ? style : 3,         // سبکِ سفارشی (پیش‌فرض LineStyle.LargeDashed)
    labelVisible: m.linesVisible && labels,
 ...
 */
export const paintCrosshairGlyph = (ctx, glyph, x, y, th, colorOverride) => {
  if (!ctx || glyph === 'none' || x == null || y == null) return;
  const color = colorOverride || (th && th.accent) || '#2962FF';
  ctx.save();
  if (glyph === 'dot') {
    ctx.beginPath();
...
export const paintSessions = (ctx, chart, bands, height) => {
  ...
  for (const b of bands) {
    ...
    ctx.stroke();
    // برچسبِ نامِ سشن بالای باند (فقط اگر پهنای باند کافی است تا شلوغ نشود)
    if (b.label && w > 46) {
      ctx.font = '9px Vazirmatn, sans-serif'; ctx.fillStyle = b.edge; ctx.textBaseline = 'top'; ctx.textAlign = 'center';
      ctx.fillText(b.label, (x1 + x2) / 2, 3);
    }
  }
  ctx.restore();
};
