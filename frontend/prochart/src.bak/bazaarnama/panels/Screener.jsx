...
export default function Screener({ symbol, TH, symbols = [], prices = {}, setSymbol }) {
  ...
  return (
    <div className="flex flex-col text-xs" style={{ color: TH.text, fontVariantNumeric: 'tabular-nums' }}>
      ...
      {/* تب‌های دستهٔ دارایی */}
      <div className="flex items-center gap-1 px-2 py-1 border-b overflow-x-auto shrink-0" style={{ borderColor: TH.border }}>
        {CATS.map(([k, label]) => (
          <button key={k} onClick={() => setCat(k)}
            className="px-2 py-0.5 rounded-md text-[10px] whitespace-nowrap transition-colors shrink-0"
            style={cat === k ? { background: TH.accent, color: '#fff' } : { background: TH.chipBg, color: TH.text }}
