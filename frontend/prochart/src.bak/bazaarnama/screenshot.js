...
export async function captureChart(o) {
  ...
  if (caption) {
    ...
    ctx.font = '600 13px AnjomanMax, Vazirmatn, sans-serif';
  ...
  if (watermark && watermark.enabled !== false && watermark.src) {
    ...
    if (img) {
      ...
      if (wantText && watermark.text) {
        ...
        ctx.font = '700 13px Vazirmatn, sans-serif'; ctx.textBaseline = 'top'; ctx.textAlign = 'left';
      ...
      ctx.fillStyle = watermark.invert ? '#000' : '#fff';
      ctx.font = '700 15px Vazirmatn, sans-serif'; ctx.textBaseline = 'top'; ctx.textAlign = 'left';
