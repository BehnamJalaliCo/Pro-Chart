...
var ratingLabel = (s) => s > 0.5 ? "strongBuy" : s > 0.1 ? "buy" : s < -0.5 ? "strongSell" : s < -0.1 ? "sell" : "neutral";
function momentumSignal(closes, len = 10) {
...
function adxSignal(adx2, plusNow, minusNow, plusPrev, minusPrev) {
  if (![adx2, plusNow, minusNow, plusPrev, minusPrev].every(Number.isFinite)) return 0;
  if (adx2 > 20 && plusPrev < minusPrev && plusNow > minusNow) return 1;
  if (adx2 > 20 && plusPrev > minusPrev && plusNow < minusNow) return -1;
  return 0;
}
function bullBearSignal(bullNow, bullPrev, bearNow, bearPrev) {
...
function technicalRating(candles) {
  try {
    ...
    const rateMA = (v) => {
      ...
      maTot++;
      if (price > v) maBuy++;
      else if (price < v) maSell++;
    };
    [10, 20, 30, 50, 100, 200].forEach((p) => {
      if (candles.length <= p + 2) return;
      rateMA(lastN(sma(closes, p)));
      rateMA(lastN(ema(closes, p)));
    });
    ...
    const r = lastN(rsi(closes, 14));
    if (r != null) bump(r < 30 ? 1 : r > 70 ? -1 : 0);
    const st = stoch(highs, lows, closes, 14, 3);
    const k = lastN(st && st.k);
    if (k != null) bump(k < 20 ? 1 : k > 80 ? -1 : 0);
    const cc = lastN(cci(highs, lows, closes, 20));
    if (cc != null) bump(cc < -100 ? 1 : cc > 100 ? -1 : 0);
    const m = macd(closes, 12, 26, 9);
    const ml = lastN(m && m.macd), ms = lastN(m && m.signal);
    if (ml != null && ms != null) bump(ml > ms ? 1 : ml < ms ? -1 : 0);
    const wr = lastN(williamsR(highs, lows, closes, 14));
    if (wr != null) bump(wr < -80 ? 1 : wr > -20 ? -1 : 0);
    const aoArr = ao(highs, lows);
    const av = lastN(aoArr);
    if (av != null) bump(av > 0 ? 1 : av < 0 ? -1 : 0);
    const sr = stochRsi(closes, 14, 3, 3);
    const srk = lastN(sr && sr.k);
    if (srk != null) bump(srk < 20 ? 1 : srk > 80 ? -1 : 0);
    ...
    const uv = lastN(uo && uo.line);
    if (uv != null) bump(uv > 70 ? 1 : uv < 30 ? -1 : 0);
    const ax = adxDI(highs, lows, closes, 14);
    if (ax) {
      const A = ax.adx, P = ax.plusDI, M = ax.minusDI, na = A.length;
      bump(adxSignal(A[na - 1], P[na - 1], M[na - 1], P[na - 2], M[na - 2]));
    }
    const er = elderRay(highs, lows, closes, 13);
    if (er && er.bull && er.bear) {
      const B = er.bull, Be = er.bear, nb = B.length;
    ...
    return {
      ...
      maLabel: ratingLabel(maScore),
      oscLabel: ratingLabel(oscScore),
      buy: maBuy + oBuy,
      sell: maSell + oSell,
      neutral: maTot - maBuy - maSell + (oTot - oBuy - oSell),
      // شمارشِ تفکیکیِ هر بخش (سبکِ Technicalsِ TV که برای «Moving Averages» و «Oscillators» جداگانه Sell/Neutral/Buy می‌دهد)
      maBuy,
      maSell,
      maNeutral: maTot - maBuy - maSell,
      oscBuy: oBuy,
      oscSell: oSell,
      oscNeutral: oTot - oBuy - oSell
    };
  } catch (e) {
    return null;
  }
