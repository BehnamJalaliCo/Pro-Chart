...
// آدرسِ سرورِ مستقلِ Pro-Chart (بازارنمای تمام‌صفحه). فعلاً IP؛ پس از آماده‌شدنِ دامین → https://pro-chart.ir
const PROCHART_URL = 'http://91.107.181.124';
...
const NAV = [
  ...
  { ext: true, label: 'بازارنما', icon: CandlestickChart, vip: true },
...
export default function Layout({ children }) {
  ...
  const NavLinks = ({ onClick }) => NAV.map((n) => {
    ...
    // بازارنما: روی سرورِ مستقلِ Pro-Chart است → کلیک، تبِ جدید را با توکنِ کاربر باز می‌کند.
    if (n.ext) {
      if (locked) {
        return (
          <Link key="bazaarnama" to="/subscribe" onClick={onClick}
            className="px-3 py-2 rounded-lg text-sm flex items-center gap-1.5 whitespace-nowrap shrink-0 text-text-muted hover:text-text-secondary">
            <I size={16} /> {n.label} <Lock size={11} className="text-amber-400/80" />
          </Link>
        );
      }
      const openExt = (e) => {
        ...
        window.open(PROCHART_URL + (t ? '#t=' + encodeURIComponent(t) : ''), '_blank', 'noopener,noreferrer');
        onClick && onClick();
      };
      return (
        <a key="bazaarnama" href={PROCHART_URL} target="_blank" rel="noopener noreferrer" onClick={openExt}
          className="px-3 py-2 rounded-lg text-sm flex items-center gap-1.5 whitespace-nowrap shrink-0 text-text-secondary hover:text-text-primary">
          <I size={16} /> {n.label}
        </a>
      );
    }
