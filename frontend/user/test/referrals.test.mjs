...
test('departure component requires disclosure acknowledgement and a native GET form', async () => {
  const [component, css] = await Promise.all([
    source('frontend/user/src/components/ReferralDeparture.jsx'),
    source('frontend/user/src/index.css'),
  ]);
  ...
  assert.match(component, /referral-departure-submit/);
  assert.match(css, /\.referral-departure-submit:not\(:disabled\) \{ background-color: #1565c0; \}/);
  assert.match(css, /\.referral-departure-submit:not\(:disabled\):hover \{ background-color: #0d47a1; opacity: 1; \}/);
