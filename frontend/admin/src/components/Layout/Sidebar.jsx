...
const navItems = [
  ...
  { to: '/signals', label: 'سیگنال‌ها', icon: Signal },
  { to: '/backtest', label: 'بک‌تست', icon: TestTube2 },
  { to: '/risk', label: 'مدیریت ریسک', icon: ShieldAlert },
  { to: '/users', label: 'کاربران', icon: Users },
  { to: '/performance', label: 'عملکرد', icon: TrendingUp },
  { to: '/reports', label: 'گزارش‌های پیشرفته', icon: BarChart3 },
  { to: '/ml-models', label: 'مدل‌های ML', icon: Brain },
  { to: '/monitoring', label: 'مانیتورینگ', icon: Monitor },
  { to: '/visitors', label: 'بازدیدکنندگان', icon: Eye },
  { to: '/seo', label: 'سئو و عملکرد', icon: Search },
  { to: '/articles', label: 'مقالات', icon: FileText },
  { to: '/broadcasts', label: 'پیام‌ها', icon: Send },
  { to: '/live', label: 'لایو ترید', icon: Radio },
  { to: '/auto-trade', label: 'اتو ترید', icon: Bot },
  { to: '/trade-history', label: 'تاریخچهٔ سود/زیان', icon: BarChart3 },
  { to: '/panel-users', label: 'کاربران پنل', icon: UserCog },
  { to: '/academy', label: 'آکادمی VIP', icon: GraduationCap },
  { to: '/ig-users', label: 'کاربران IG', icon: Instagram },
  ...
  return (
    <>
      <div className="flex items-center gap-3 px-4 h-16 border-b border-surface-border shrink-0">
        <div className="w-9 h-9 rounded-lg bg-brand-blue flex items-center justify-center text-white font-bold text-sm shrink-0">
          FX
        </div>
        {!collapsed && (
          <div className="overflow-hidden flex-1">
            <h1 className="text-sm font-bold text-text-primary truncate">سیگنال فارکس</h1>
