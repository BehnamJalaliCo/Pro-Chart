...
int OnInit()
{
   ...
   Print("CoineProAutoTrader v1.3 — Magic=", MagicNumber, " commonFlag=", g_commonFlag);
...
void OpenNew()
{
   ...
   for(int i=0;i<ArraySize(g_sigs);i++)
   {
      ...
      if(ok) { g_dayTrades++; MarkOpened(s.id); Print("باز شد ",s.action," ",sym," lots=",lots," #",s.id); }
...
void ManageOpen()
{
   for(int i=PositionsTotal()-1;i>=0;i--)
   {
      ...
      if(newSL!=curSL && newSL>0){
         ...
         if((type==POSITION_TYPE_BUY && newSL<price) || (type==POSITION_TYPE_SELL && newSL>price))
